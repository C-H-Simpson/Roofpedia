...
year = 2019
predictions = gpd.read_file(f"results/buffered_polygons_{year}.geojson").to_crs(native_crs)
...
for name in results:
    for i in (0, 1, 2):
        ax[i].hist(results[name][i], label=name, bins=np.arange(40, 220, 1), histtype="step")
ax[0].set_xlabel("Red")
...
for name in results:
    ...
    for i in (0, 1, 2):
        ax[i].hist(ness[i], label=name, bins=np.linspace(0.25, .40, 50), histtype="step")
    ax[3].hist(results[name].mean(0), label=name, bins=np.arange(50, 200, 5), histtype="step")
...
recall = keep("tp").sum() / (keep("tp").sum()+(~keep("tp")).sum()+ness["fn"].shape[1])
f1 = 2*(precision*recall)/(precision+recall)
print(precision, recall, f1)
