...
for config in kfold_config_paths + ["config/best-predict-config.toml"]:
    ...
    for name in ("validation", "training_s", "training_b", "testing", "testing_alt", "validation_alt"):
        ...
        gdf_tiles_signal = gdf_tiles.sjoin(predictions[["geometry"]].assign(pred=True)).pipe(lambda _df: _df[_df.pred==True])[["geometry"]]
        ...
        if name != "training_b":
            ...
            union_buildings = np.union1d(truth_buildings, pred_buildings)
            tn_pos_count = np.setdiff1d(gdf_tiles.sjoin(ukb).geomni_premise_id.unique(), union_buildings).shape[0]
        ...
        results.append({
            ...
            "fp_pos_count": fp_pos_count, "tp_pos_count": tp_pos_count, "fn_pos_count": fn_pos_count, "tn_pos_count": tn_pos_count,
...
df["precision_pos_count"] = df.tp_pos_count / (df.tp_pos_count + df.fp_pos_count + epsilon)
df["recall_pos_count"] = df.tp_pos_count / (df.tp_pos_count + df.fn_pos_count + epsilon)
df["f1_pos_count"] = 2*(df.precision_pos_count*df.recall_pos_count)/(df.precision_pos_count+df.recall_pos_count + epsilon)
