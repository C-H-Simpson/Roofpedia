...
# for erosion in (-2, -1,  0):
    # for truth_expansion in (0, 1, 2):
        ...
        # Apply operations
        predictions = gpd.GeoDataFrame(geometry=list(predictions.unary_union.geoms), crs=native_crs)
        if simplify!=0:
...
# %%
print(df[["erosion", "simplify", "precision", "recall", "f1"]].sort_values("f1").to_string())
# %%
fig, ax = plt.subplots()
